...
def encoded_command_execution(earliest: str = None, latest: str = None, target_user: str = None):
    ...
    qh_filtered = qh.filter(
        ...
        (
            ...
            (
                (col("statement_text").contains("base64") & col("statement_text").contains("--decode")) |
                (col("statement_text").contains("base64") & col("statement_text").contains("-d "))
            ) |
            # printf hex decode: printf "%s" "<hex>" pattern used to pass encoded payloads
            (col("statement_text").contains("printf") & col("statement_text").rlike(r"[0-9a-fA-F]{20,}"))
        )
    )
    ...
    audit_filtered = audit.filter(
        ...
        (col("service_name") == "notebook") &
        (col("action_name") == "runCommand") &
        (
            ...
            (
                (col("request_params.commandText").contains("base64") &
                 col("request_params.commandText").contains("--decode")) |
                (col("request_params.commandText").contains("base64") &
                 col("request_params.commandText").contains("-d "))
            ) |
